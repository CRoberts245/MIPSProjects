...
li $v0, 4
la $a0, failPrompt            #if user enters < 0, reprompt
syscall
...
sll $t1, $t0, 2
add $t1, $t1, $s0
move $a0, $t1                    #store arguments for insertion
move $a1, $v0



jal insertion                    #go to insertion
...
li $v0, 4
la $a0, searchPrompt              #prompt user for search value
syscall

li $v0, 5                          #take in user search int
syscall

li $t9, -999                       #load exit value
...
move $a0, $v0
li $v0, 1                           #output return value
syscall

beq $a0, $zero, NEXT1               #jump to NEXT1 if binarysearch returns 0
li $v0, 4
la $a0, valueFound                  #prompt user that value was found
syscall
...
NEXT1:
li $v0, 4
la $a0, valueNotFound              #prompts user value not found
syscall
...
li $v0, 4
la $a0, exitPrompt                  #tell user have a nice day!
syscall
...
sw $t3, 0($a0)
addi $a0, $a0, -4
j insertion                     #repeat insertion attempt
